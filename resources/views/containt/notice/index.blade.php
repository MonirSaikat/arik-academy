        ...
        <div class="row mt-3">
            <div class="col-lg-12">
                @if (userHasPermission('class-store'))
                    <button type="button" class="btn btn-sm btn-primary mr-2 mb-3" data-toggle="modal"
                        data-target="#AddModel">
                        <i class="fa fa-plus"></i> Add School Notice
                    </button>
                @endif
                <div class="card mb-4">
                            ...
                            <tbody>
                                @foreach ($notices as $key => $notice)
                                    <tr>
                                        ...
                                        <td>{{ date('Y-m-d') }}</td>
                                        <td>{{ $notice->notices_description }}</td>
                                        {{-- <td>{{$notice->file}}</td> --}}
                                        <td>
                                            <img src="{{ asset('uploads/notices/' . $notice->file) }}" width="50px">
                                        </td>
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu">
                                                    ...
                                                    @if (userHasPermission('session-delete'))
                                                        <a class="dropdown-item text-danger"
                                                            href="{{ route('school.notice.delete', $notice->id) }}"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('school.notice.store') }}" method="post" enctype="multipart/form-data">
                        @csrf

                        <div class="mb-3">
                            <label for="description" class="form-label">Notice Title</label>
                            <input type="text" class="form-control" id="description" name="description" required>
                        </div>


                        <div class="mb-3">
                            <label for="notices_description" class="form-label">Notice Description</label>
                            <textarea name="notices_description" id="notices_description" class="form-control"></textarea>
                        </div>

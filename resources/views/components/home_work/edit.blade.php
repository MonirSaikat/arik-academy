  ...
  <div class="container-fluid" id="container-wrapper">
    ...
    <div class="row mt-3">
      <div class="col-md-12 mb-3">
        <form action="{{ route('backend.home_work.update', $homework->id) }}" method="POST"
          enctype="multipart/form-data">
          @csrf
          <div class="row">
            <div class="col-md-3 mb-3">
              <label for="class_id">Class</label><span class="ml-2">*</span>
              <select name="class_id" id="class_id" required class="form-control selectpicker" data-live-search="true"
                title="Select Class" onchange="getSection(this.value)">
                @foreach (App\Models\Academic\Classes::where('is_active', 1)->get() as $item)
                  <option value="{{ $item->id }}" {{ $item->id == $homework->class_id ? 'selected' : '' }}>
                    {{ $item->name }}</option>
                @endforeach
              </select>
            ...
            <div class="col-md-3 mb-3" id="group"
              style="display: {{ $homework->group_id != null ? 'block' : 'none' }}">
              <label for="group_id">Group</label>
              <select name="group_id" id="group_id" class="form-control selectpicker" data-live-search="true"
                title="Select Group">
                @foreach ($group as $item)
                  <option value="{{ $item->id }}" {{ $item->id == $homework->group_id ? 'selected' : '' }}>
                    {{ $item->name }}</option>
                @endforeach
              </select>
              <span class="text-light">
            ...
            <div class="col-md-3 mb-3" id="section"
              style="display: {{ $homework->section_id != null ? 'block' : 'none' }}">
              <label for="section_id">Section</label>
              <select name="section_id" id="section_id" class="form-control selectpicker" data-live-search="true"
                title="Select Section">
                @foreach ($section as $item)
                  <option value="{{ $item->id }}" {{ $item->id == $homework->section_id ? 'selected' : '' }}>
                    {{ $item->name }}</option>
                @endforeach
              </select>
              <span class="text-light">
            ...
            <div class="col-md-3 mb-3">
              ...
              <select name="subject_id" id="subject_id" required class="form-control selectpicker" data-live-search="true"
                title="Select Subject">
                @foreach ($subject as $item)
                  <option value="{{ $item->id }}" {{ $item->id == $homework->subject_id ? 'selected' : '' }}>
                    {{ $item->name }}</option>
                @endforeach
              </select>
              <span class="text-light">
            ...
            <div class="col-md-3 mb-3">
              ...
              <input type="date" name="homework_date" required class="form-control" id="homework_date"
                value="{{ $homework->homework_date }}">
              </select>
              <span class="text-light">
                @error('homework_date')
                  {{ $message }}
            ...
            <div class="col-md-3 mb-3">
              ...
              <input type="date" name="submission_date" required class="form-control" id="submission_date"
            ...
            <div class="col-md-3 mb-3">
              ...
              <input type="text" name="mark" required value="{{ $homework->mark }}" id="mark"

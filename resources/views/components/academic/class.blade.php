                    ...
                    <tr>
                      ...
                      <td>
                        @foreach ($item->groups as $items)
                          <span class="bg-primary text-light rounded p-1">{{ $items->name }}</span>
                        @endforeach
                      </td>
                      <td>
                        @foreach ($item->section as $items)
                          <span class="bg-primary text-light rounded p-1">{{ $items->name }}</span>
                        @endforeach
                      </td>
                      <td>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="{{ route('backend.class.store') }}" method="post">
            @csrf
            <div class="row">
              ...
              <div class="col-md-6">
                <fieldset>
                  <legend>Choose Group</legend>
                  @foreach (App\Models\Academic\Group::where('is_active',1)->get() as $item)
                    <div>
                      <input type="checkbox" id="{{ $item->name }}" name="group[]" value="{{ $item->id }}" />
                      <label for="{{ $item->name }}">{{ $item->name }}</label>
                    </div>
                  @endforeach
                </fieldset>

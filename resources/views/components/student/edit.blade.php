    ...
    <div class="container-fluid" id="container-wrapper">
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            ...
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item" aria-current="page">Create Edit</li>
            </ol>
        </div>

        <form action="{{ route('backend.student.update', $student->id) }}" method="POST" enctype="multipart/form-data">
            <div class="row mt-3 bg-primary text-light py-3 rounded">
                @csrf
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <select name="class_id" id="class_id" class="form-control selectpicker" data-live-search="true"
                        title="Select Class" onchange="getSection()">
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <select name="session_id" id="session_id" class="form-control selectpicker" data-live-search="true"
                        title="Select Section">
                        @php
                            $sessions = App\Models\Academic\Session::where('is_active', 1)->get();
                        @endphp
                        @foreach ($sessions as $item)
                            <option value="{{ $item->id }}" {{ $item->id == $student->session_id ? 'selected' : '' }}>
                                {{ $item->name }}</option>
                        @endforeach
                    </select>
                    <span class="text-light">
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <select name="religion" id="religion" class="form-control selectpicker" data-live-search="true"
                        title="Select Section">
                        @foreach (App\Models\Religion::all() as $item)
                            <option value="{{ $item->id }}" {{ $item->id == $student->religion ? 'selected' : '' }}>
                                {{ $item->name }}</option>
                        @endforeach
                    </select>
                    <span class="text-light">
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <select name="gender" id="gender" class="form-control selectpicker" data-live-search="true"
                        ...
                        @foreach (App\Models\Gender::all() as $item)
                            <option value="{{ $item->id }}" {{ $item->id == $student->gender ? 'selected' : '' }}>
                                {{ $item->name }}</option>
                        @endforeach
                    </select>
                    <span class="text-light">
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <select name="blood_group" id="blood_group" class="form-control selectpicker"
                        data-live-search="true" title="Select Blood Group">
                        @foreach (App\Models\BloodGroup::all() as $item)
                            <option value="{{ $item->name }}"
                                {{ $item->name == $student->blood_group ? 'selected' : '' }}>{{ $item->name }}</option>
                        @endforeach
                    </select>
                    <span class="text-light">
                        @error('blood_group')
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <input type="text" name="birth_certificate_number" id="birth_certificate_number"
                        class="form-control" value="{{ $student->birth_certificate_number }}">
                    <span class="text-light">
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <img src="{{ asset('images/students/') . '/' . $student->photo }}" alt="Photo" width="60px"
                        height="50px">
                </div>
                ...
                <div class="col-md-12 mt-3 d-flex">
                    <h4>Permanent Address</h4><strong class="ml-3 mr-1" style="line-height: 38px">Same </strong> <input
                        type="checkbox" id="same_address" name="same_address">
                </div>
                <div class="col-md-3 mb-3">
                    ...
                    <input type="text" name="parmanent_district" id="parmanent_district" class="form-control"
                        value="{{ $student->parmanent_district }}">
                </div>
                <div class="col-md-3 mb-3">
                    ...
                    <input type="text" name="parmanent_upozila" id="parmanent_upozila" class="form-control"
                        value="{{ $student->parmanent_upozila }}">
                </div>
                <div class="col-md-3 mb-3">
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <input type="text" name="parmanent_village" id="parmanent_village" class="form-control"
                        value="{{ $student->parmanent_village }}">
                </div>
                <div class="col-md-12">
                    <input type="submit" class="btn btn-warning text-dark" value="Update">
                </div>
            </div>
        </form>
    </div>
        ...
        function getSection() {
            ...
            if (id) {
                ...
                section.filter(function(item) {
                    ...
                    $('#section_id').append(
                        `<option ${student.seciton_id == item.id ? 'selected': ''} value="${item.id}">${item.name}</option>`
                    );
                    i++;
                });
                ...
                $('#section_id').val(student.section_id);

...
<section>
  <div class="container-fluid">
      <button class="btn btn-info" data-toggle="modal" data-target="#createModal"><i class="fa fa-plus"></i>Designation</button>
  </div>
<div class="card mt-3">
  <div class="table-responsive p-3">
      <table id="biller-table" class="table table-striped">
        ...
        @foreach($designation as $key=>$designations)
           <tr data-id="">
                <td>{{$key+1}}</td>
                <td>{{$designations->employee_name}}</td>
                  ...
                  <td>
                    <div class="dropdown">
                      ...
                      <div class="dropdown-menu">
                        ...
                        <a class="dropdown-item" href="javascript:void(0)" onclick="editbutton({{$designations->id}})"><i class="fas fa-edit mr-2"></i> Edit</a>
                        @endif
                        @if (userHasPermission('session-delete'))
                        <a class="dropdown-item text-danger" href="{{route('backend.designation.delete',$designations->id)}}" onclick="return confirm('Are you sure to delete this data..??')"><i class="fas fa-trash mr-2"></i>Delete</a>
                        @endif
                      </div>
                    </div>
                  </td> 
...
<div role="document" class="modal-dialog">
  ...
  <div class="modal-content">
     <form action="{{route('backend.designation.update')}}" method="post">
      @csrf
      <div class="modal-header">
    ...
    <div class="form-group">
        <label><strong>Designation*</strong></label>
        <input class="form-control" name="designation" id="update_designation">
    </div> 
...
function editbutton(Id){
  ...
  var url ='{{route("backend.designation.edit", ":id")}}'
  ...
  $.ajax({
      ...
      success:function(response){
          ...
          $('#update_designation').val(response.designation);

...
@section('title','Optional Subject Assign')
@section('content')
...
<div class="container-fluid" id="container-wrapper">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
      <h1 class="h3 mb-0 text-gray-800">
        Optional Subject Assign
      </h1>
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{route('backend.dashboard')}}">Home</a></li>
        <li class="breadcrumb-item" aria-current="page">Optional Subject Assign</li>
      </ol>
    </div>
    ...
    <div class="row mt-3">
        <div class="col-md-12 mb-3">
            <form action="{{ route('backend.subjectassignstudent.optionalassign') }}" method="GET">
                <div class="row">
        ...
        <div class="col-md-12 mt-3">
            <div class="card mb-4">
                ...
                <form action="{{ route('backend.subjectassignstudent.optionalassignstore') }}" method="POST">
                    @csrf
                  <table class="table align-items-center table-flush table-hover table-striped">
                    <thead class="bg-primary text-light">
                      <tr>
                        <th><input type="checkbox" id="all_permission" class="mr-2"><label for="all_permission">All Check</label></th>
                        <th>Roll</th>
                        <th>Name</th>
                        <th>class</th>
                      ...
                      @foreach ($student as $key=>$item)
                          <tr>
                            <td style="width: 150px">
                                <input type="checkbox" name="student[]" id="student" value="{{ $item->id }}" class="mr-2">
                            </td>
                    ...
                    <tfoot class="bg-light text-dark">
                      <tr>
                        ...
                        <td>
                          <select name="subject" id="subject" class="form-control selectpicker" title="Select Subject" required>
                            @foreach (App\Models\Subject\Subject::where('type',2)->get() as $item)
                                <option value="{{ $item->id }}">{{ $item->name }}</option>
                            @endforeach
                          </select>
                        </td>

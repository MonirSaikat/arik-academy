...
@section('title','Income_Category')
@section('content')
<section>
  <div class="container-fluid">
      <button class="btn btn-info" data-toggle="modal" data-target="#createModal"><i class="fa fa-plus"></i>Income Category </button>
  </div>
<div class="card mt-3">
                  ...
                  <td>
                    <div class="dropdown">
                      ...
                      <div class="dropdown-menu">
                        ...
                        <a class="dropdown-item" href="javascript:void(0)" onclick="editbutton({{$incomes->id}})"><i class="fas fa-edit mr-2"></i> Edit</a>
                        @endif
                        @if (userHasPermission('session-delete'))
                        <a class="dropdown-item text-danger" href="{{route('backend.income_category.delete',$incomes->id)}}" onclick="return confirm('Are you sure to delete this data..??')"><i class="fas fa-trash mr-2"></i>Delete</a>
                        @endif
                      </div>
                    </div>
                  </td> 
...
<div id="createModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
  <div role="document" class="modal-dialog">
    <div class="modal-content">
      <form action="{{route('backend.income_category.store')}}" method="post">
        @csrf
      <div class="modal-header">
        <h5 id="exampleModalLabel" class="modal-title">Add income category</h5>
...
<div role="document" class="modal-dialog">
  ...
  <div class="modal-content">
     <form action="{{route('backend.income_category.update')}}" method="post">
      @csrf
      <div class="modal-header">
...
function editbutton(Id){
  ...
  var url ='{{route("backend.income_category.edit", ":id")}}'
